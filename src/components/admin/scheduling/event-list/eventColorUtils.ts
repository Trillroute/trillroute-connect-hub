...
export const getEventColor = (event: CalendarEvent) => {
  // For availability slots, use category-based coloring like other views
  if (event.eventType === 'availability') {
    ...
    switch (category) {
      ...
      default:
        return '#10B981'; // Green for session availability - matches EventColorPicker
    }
  }
...
export const getEventTypeBackgroundClass = (event: CalendarEvent) => {
  // For availability slots, use category-based styling
  if (event.eventType === 'availability') {
    const category = event.metadata?.category?.toLowerCase();
    switch (category) {
      case 'break':
      ...
      default:
        return 'bg-green-100 border-green-300 text-green-800'; // Green for session availability
    }
  }
